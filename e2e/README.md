...
| `llaves.mjs` | El **registro de llaves** de punta a punta, con clicks reales: cargar una llave, abrir y cerrar SU historial (acordeón), entregarla por el modal del sistema (y que la lista diga A QUIÉN — el corazón del pedido de Mateo), devolverla y borrarla confirmando en el modal. Vigila que NO haya vuelto ningún `window.prompt`/`confirm` del navegador. Se limpia sola (llave 8801). El aislamiento entre oficinas es de la base (`verificar-db`) | sesión real | 19 |
