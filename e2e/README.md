...
| `llaves.mjs` | El **registro de llaves** de punta a punta, con clicks reales: cargar una llave, abrir y cerrar SU historial (acordeón), entregarla por el modal del sistema (y que la lista diga A QUIÉN — el corazón del pedido de Mateo), devolverla y borrarla confirmando en el modal. Vigila que NO haya vuelto ningún `window.prompt`/`confirm` del navegador. Se limpia sola (llave 8801). El aislamiento entre oficinas es de la base (`verificar-db`) | sesión real | 21 |
| `temporada-mogotes.mjs` | Los **seis pedidos de Mateo del 13-ago** sobre Temporada, a 390 y 1440: que la tarjeta lleve a **la ficha de la propiedad** (fotos + descripción) y no al barrio, que la ficha diga **Temporada** y muestre el **precio por quincena**, que el único filtro sea **cuántas personas**, y los tres textos nuevos. ⚠️ Mira el CONTENEDOR de cada cosa, no el body entero: «quincena» vive también en el precio de cada tarjeta y «en qué zona querés estar» en el CTA final — filtrar de más da falsos rojos | sesión real | 26 |
| `confirmaciones.mjs` | Que **ningún borrado se ejecute sin preguntar**, en Clientes, Tasaciones y Cartera. Por pantalla: que pregunte con un modal NUESTRO, que no haya llamado a un diálogo del navegador (se los sobreescribe para cazarlos) y 🔑 **que CANCELAR no borre nada** — verificado contra la cartera REAL de Mateo (100 → 100). Existe porque `window.confirm` bloqueaba y el modal es asíncrono: un efecto fuera del `onOk` borra sin confirmación | sesión real | 10 |
| `rendimiento.mjs` | ⏱️ Que la LENTITUD la cace una prueba y no el cliente. Topes de tiempo para la home, el catálogo, la ficha y el 3D (montaje + primera sombra), más el peso del bundle principal. Nació el 14-ago: Juani abrió el 3D y tardó 30-40 s por el cold start de Render — un problema **ya anotado** como bloqueante nº1 que nadie estaba midiendo, así que el aviso llegó por WhatsApp. Los topes son ~1,6× lo medido en producción despierta; si algo los pasa, empeoró de verdad. ⚠️ Contra Render hay que calentar el servicio antes: si da rojo en todo, el problema es el hosting | visitante | 6 |
...
| `privacidad-perfiles.mjs` | Que una oficina no pueda ponerse el sombrero de la Dirección, ni recreando perfiles con ids al azar ni editando `localStorage` a mano | sesión real | 21 |
